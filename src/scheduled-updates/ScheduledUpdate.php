...
class ScheduledUpdate {
    ...
    public static function register_post_status(): void
    {
        ...
        register_post_status(ScheduledUpdate::$_publish_status, $args);
    }
    ...
    public static function display_post_states( $states ): array
    {
        ...
        if ( $arg !== ScheduledUpdate::$publish_label && $post->post_status === ScheduledUpdate::$_publish_status ) {
            $states = array( ScheduledUpdate::$publish_label );
            if ( ! $type->hierarchical ) {
                $orig = get_post(get_post_meta($post->ID, ScheduledUpdate::$_publish_status . '_original', true));
    ...
    public static function page_row_actions( $actions, $post ): array
    {
        ...
        if ( $post->post_status === ScheduledUpdate::$_publish_status ) {
            ...
            $actions['copy_to_publish'] = '<a href="' . admin_url('admin.php' . $copy) . '">' . ScheduledUpdate::$publish_label . '</a>';
        }
    ...
    public static function manage_pages_custom_column( $column, $post_id ): void
    {
        if ( 'publish' === $column ) {
            ...
            if ( $stamp ) {
                echo esc_html(ScheduledUpdate::get_pubdate($stamp));
            }
        }
    }
    ...
    public static function add_meta_boxes_page( $post_type, $post ): void
    {
        ...
        wp_enqueue_script(ScheduledUpdate::$_publish_status . '-datepicker.js', plugins_url('js/publish-datepicker.js', __FILE__), array('jquery-ui-datepicker'));
        ...
        $js_data = array(
            'datepicker' => array(
                'daynames'   => $days,
                'monthnames' => $months,
                'elementid'  => ScheduledUpdate::$_publish_status . '_pubdate',
                'displayid'  => ScheduledUpdate::$_publish_status . '_pubdate_display',
                'dateformat' => $df,
            ),
            'text' => array(
                'save' => __('Save'),
    ...
    public static function create_meta_box( $post ): void
    {
        ...
        $dateo = new DateTime('now', ScheduledUpdate::get_timezone_object() );
        if ( $stamp ) {
        ...
        <p>
            ...
            echo sprintf( __( 'Please enter <i>Time</i> as %s', 'scheduledupdate-td' ), ScheduledUpdate::get_timezone_string() ); // WPCS: XSS okay.
            ?>
        </p>
        <p>
    ...
    public static function prevent_status_change( $new_status, $old_status, $post ): void
    {
        ...
        if ( $old_status === ScheduledUpdate::$_publish_status && 'trash' !== $new_status ) {
            remove_action('save_post', array('ScheduledUpdate', 'save_meta'), 10);

            $post->post_status = ScheduledUpdate::$_publish_status;
        ...
        } elseif ( 'trash' === $old_status && $new_status === ScheduledUpdate::$_publish_status ) {
            wp_schedule_single_event(get_post_meta($post->ID, ScheduledUpdate::$_publish_status . '_pubdate', true), 'publish_post', array(
                'ID' => $post->ID,
            ));
        }
    }
    ...
    public static function create_publishing_post( $post ): int
    {
        ...
        $original = $post->ID;
        if ( $post->post_status === ScheduledUpdate::$_publish_status ) {
            $original = get_post_meta($post->ID, ScheduledUpdate::$_publish_status . '_original', true);
        }
        ...
        $new_post = array(
            ...
            'post_status'    => ScheduledUpdate::$_publish_status,
        ...
        ScheduledUpdate::copy_meta_and_terms( $post->ID, $new_post_id );

        // and finally referencing the original post.
        update_post_meta($new_post_id, ScheduledUpdate::$_publish_status . '_original', $original);
    ...
    public static function save_meta( $post_id, $post )
    {
        if ( $post->post_status === ScheduledUpdate::$_publish_status || get_post_meta($post_id, ScheduledUpdate::$_publish_status . '_original', true)) {
            ...
            if ( isset( $_POST[ $pub ] ) && isset( $_POST[ $pub . '_time_hrs' ] ) && isset( $_POST[ $pub . '_time_mins' ] ) && ! empty( $_POST[ $pub ] ) ) {
                $tz = ScheduledUpdate::get_timezone_object();
    ...
    public static function publish_post( $post_id ): int
    {
        ...
        ScheduledUpdate::copy_meta_and_terms( $post->ID, $orig->ID );
        ...
        delete_post_meta($orig->ID, ScheduledUpdate::$_publish_status . '_original');
        delete_post_meta($orig->ID, ScheduledUpdate::$_publish_status . '_pubdate');
